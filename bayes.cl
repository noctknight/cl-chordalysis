...
(defparameter *datatype* :csv
  "Either :ALLEGROGRAPH or :CSV at the moment.")

(defparameter *db-name* "data/protein.csv"
  "This is the name of the CSV file for this version.")
...
(defmethod column-position (header-name (data csv-data))
  (position header-name (headers data) :test #'string-equal))

(defmethod print-object ((csv csv-data) stream)
  (print-unreadable-object (csv stream :type t)
    (format stream "with ~D columns and ~D rows"
            (length (headers csv))
            (length (rows csv)))))

(defun load-csv (&key (path (merge-pathnames *db-name* *base-dir*))
                   (max-nr-columns 80))
  (flet ((to-vector (list)
...
(defun init-cpt-for-node (node)
  "Create an empty frequency table for this node, with the dimensions based on the
number of possible values of its parents and of itself."
  ;; The table will have  this node's values last.
  (let (dims)
...
(defun map-graph-node-names (graph function)
  "Calls FUNCTION with each node in GRAPH."
  (loop for node being the hash-value of (graph-ht graph)
     do (funcall function node)))

(defmacro do-graph-nodes ((node graph) &body body)
  `(map-graph-node-names ,graph (lambda (,node) ,@body)))
...
(defun build-graph (&key (graphdesc nil) (db-name *db-name*) (unique-nodes nil) (cutoff 1000))
...
its parents. Predicates with more than CUTOFF distinct values are ignored."
  (if (eql *datatype* :csv)
      ...
      (progn
        ...
        #+later(delete-triples :p !<http://credoo.com/connected>)
        ;; The real work.
        (unwind-protect
             (do-build-graph graphdesc :unique-nodes unique-nodes :cutoff cutoff)
          (close-triple-store)))))
...
(defun do-build-graph (graphdesc &key (unique-nodes nil) cutoff)
  "GRAPHDESC is a list of node descriptors, where each node descriptor has the form
\(CHILD-NAME &REST PARENTS)."
  (let ((predicatelists 
	 (mapcar #'(lambda (predlist)
		     ;; Prepend the terse name to speed up selection.
                     (destructuring-bind (name &rest possible-values)
                         predlist
                       (declare (ignore possible-values))
                       (cons (if (eql *datatype* :csv)
                                 (intern name)
                                 (part->terse name))
                             predlist)))
		 (select-predicates cutoff nil))))
...
(defun ensure-node (name realname ht predlists origname-ht)
  "Make sure that the node called NAME is present in the HT and ORIGNAME-HT hash tables.
PREDLISTS contains lists of the form (TERSE-NAME NAME &REST VALUES).  TERSE-NAME is either
an interned symbol (for CSV data) or the result of part->terse (for AG data). NAME is
either a string (for CSV data) or a UPI (for AG data)."
  (or (gethash name ht) ; already present
      (let ((predinfo (assoc realname predlists :test #'string-equal)))
	(when (null predinfo)
          (error "unknown predicate ~s~%" realname))
	(setf (gethash realname origname-ht) name)
	(destructuring-bind (terse upi &rest possible-values)
            predinfo
...
(defun generate-cpt (graph &key (db-name *db-name*))
  "Compute frequency tables for all nodes in GRAPH."
  ;; Initialize all frequency tables.
  (ecase *datatype*
    (:csv
     (do-graph-nodes (node graph)
       (compute-node-frequency-table node)))
    (:allegrograph
     (do-graph-nodes (node graph)
...
(defun compute-node-frequency-table (node &key (verbose t))
  (init-cpt-for-node node)
  (when verbose
    (format t "~&Generating frequency table for ~S with ~D parents, dimensions ~S, df ~S.~%"
  ...
  (ecase *datatype*
    ...
    (:allegrograph
     ...
     (let ((preds (cons (node-upi node)
       ...
       (tslet ((q -person !rdf:type !<http://credoo.com/Person>)) ; Collect needed info for person.
              (let (info)
                (dolist (pred preds)
                  (tslet ((q person pred -obj))
                         (push (cons pred obj) info)))
...
for the current row/person. HEADERS is a vector with header names and ROW is a vector with
...
#+old
(defun do-generate-cpt-one-node (node &key (verbose t))
  (init-cpt-for-node node)
  (when verbose
    (format t "~&Generating frequency table for ~S with ~D parents, dimensions ~S, df ~S.~%"
            (node-real-name node) (length (node-parents node))
...
(defun change-counts-to-probabilities (graph)
  "Destructively convert all frequency tables of GRAPH's nodes to probability tables."
  (do-graph-nodes (node graph)
...
(defun compute-edge-graph (&key (cutoff 1000))
  (ecase *datatype*
    (:csv (setq *data-source* (load-csv :path (merge-pathnames *db-name* *base-dir*)))
...
(defun create-edge-graph-inner (cutoff)
  (let ((preds ; a list of symbols with predicate names
         (ecase *datatype*
           (:csv (loop for header across (headers *data-source*)
                      collect (intern header)))
           (:allegrograph
            (mapcar #'(lambda (preddesc) (intern (part->terse (car preddesc))))
                    (select-predicates cutoff nil))))))
	...
	(let (graph-edges)
	  (do-graph-nodes (node graph)
            (when (node-parents node)
              ;; a child with a frequency table
...
(defun frequency-table-for-vertices (graph vertices)
  ...
  (let ((child (gethash (string (car vertices)) (graph-origname-ht graph)))
    ...
    (compute-node-frequency-table child)
    (node-cpt child)))
  ...
  (let ((e (pop (graph-possible-edge-list graph))))
    (cond ((adding-edge-is-ok graph
          ...
          (t
           ...
           ;; Add edge back at the end so we don't forget it and so that
           ;; ENSURE-SEPARATOR-SET can find it.
           (setf (graph-possible-edge-list graph)
...
(defun inspect-predicates (&key (cutoff 10) names)
  "Shows some details for all predicates. If NAMES is supplied, it's a list of predicate
names. Otherwise you get to see them all."
  (open-triple-store *db-name*)
  #+later(delete-triples :p !<http://credoo.com/connected>)
  (commit-triple-store)
  (unwind-protect
      (show-predicates (select-predicates cutoff names))
    (close-triple-store)))

(defun select-predicates (cutoff names)
  "Returns a list of \(PREDICATE-UPI VALUE-1 ... VALUE-N). For :CSV data sources we
just return a string with the predicate name instead of a UPI.
If NAMES is non-nil, it's a list with predicate names that we're interested in \(so all
other predicates are ignored).  CUTOFF is either NIL or an integer. If it's an integer,
predicates with more than CUTOFF possible values are ignored."
  (flet ((predicate-is-irrelevant (name)
           (and names
                (not (find name names :test #'equal)))))
    (ecase *datatype*
      (:csv (let ((table ; a table from column names (headers) to possible values
                   (make-hash-table :test 'equal))
                  (headers (headers *data-source*)))
              (loop for header across headers
                 unless (predicate-is-irrelevant header)
                 do (setf (gethash header table) '()))
              (dolist (row (rows *data-source*))
                (assert (= (length row) (length headers)))
                (loop for header across headers
                   for value across row
                   unless (predicate-is-irrelevant header)
                   do (pushnew value (gethash header table) :test #'equal)))
              (loop for predicate being the hash-key of table using (hash-value possible-values)
                 unless (and cutoff (> (length possible-values) cutoff))
                 collect (cons predicate (sort-values possible-values)))))
      (:allegrograph
         ...
         (dolist (pred preds)
           ...
           (let ((name (part->terse pred)))
             ...
             (unless (or (part= !rdf:type pred)
                         (predicate-is-irrelevant predicate)
...
(defun do-add-type-triple ()
  (dolist (item (run-sparql "select distinct ?s { ?s ?p ?o}"
                            :results-format :lists))
    (setq item (car item))
    #+later(add-triple item !rdf:type !<http://credoo.com/Person>))
  (commit-triple-store))
...
(defun generate-pymc-named-node (graph name stream)
  (let ((node (gethash name (graph-ht graph))))
	...
	(dolist (parent (node-parents node))
	  ...
	  (setq args (format nil "~A[int(~A)]" args (node-name parent)))
	  (setq before t)))
      (format stream "):~%")
