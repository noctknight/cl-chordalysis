...
(defun replace-column-values-by-quantiles (column-name data-source &key (nr-quantiles 10))
  ""
  (let* ((column (find-column-named column-name csv-data))
         (unique-values (sort (copy-list (unique-values column))
                                (sort-predicate column)))
         (quantiles (split-into-quantiles possible-values nr-quantiles)))
)

|#


(defun split-into-quantiles (values nr-quantiles)
  "Takes a vector of values and returns a vector with NR-QUANTILES lists of values
\('quantiles'). Each quantile list contains roughly the same number of values."
  (if (<= nr-quantiles 1)
      (vector (coerce values 'list))
      (let* ((nr-elts (length values))
             (max-bin-size (ceiling nr-elts nr-quantiles))
             (average-bin-size (/ nr-elts nr-quantiles))
             (result (make-array nr-quantiles :initial-element '())))
        (let ((nr-bin-elts 0)  ; nr of elements in current bin
              (bin-number 0))
          (loop for value across values
             for count from 0
             do (progn
                  (push value (aref result bin-number))
                  (incf nr-bin-elts)
                  (when (or (>= nr-bin-elts max-bin-size)
                            ;; We try to keep the current average bin size close to the
                            ;; total average bin size (which is known in advance).
                            (> (/ (1+ count) (1+ bin-number)) average-bin-size))
                    ;; Move to next bin.
                    (incf bin-number)
                    (setq nr-bin-elts 0)))))
        ;; Make sure that the values in each bin have the same order as in the original.
        (loop for i below nr-quantiles
             do (setf (aref result i) (nreverse (aref result i))))
        ;; Return result.
        result)))
...
(defclass entropy-computer ()
  ((nr-rows :initarg :nr-rows :type integer :reader nr-rows
            :documentation "The number of rows (i.e. persons, normally speaking). Petitjean:
'nbInstances - the number of lines in the database'.")
   (partial-entropy :accessor partial-entropy
                    :documentation "A vector of partial entropies \(one partial entropy
                    per row).")
))

(defun make-entropy-computer (nr-rows nr-columns)
  (let ((partial-entropy (make-array (1+ nr-rows) ; not sure why we need 1+ but that's what PetitJean does
                                     :element-type 'double-float))
        (ln-n (log nr-rows)))
    ;; Initialize partial entropy vector.
    (setf (aref partial-entropy 0) 0.0d0)
    (loop for i from 1 to nr-rows
         do (setf (aref partial-entropy i)
                  (* i (- (log i) ln-n))))
    ;; Return result.
    (make-instance 'entropy-computer
                   :nr-rows nr-rows
                   :partial-entropy partial-entropy
...
(defmethod compute-entropy (columns (computer entropy-computer))
  "Returns the entropy for a list of columns."
  (let ((cache (cache computer))
        (column-numbers (sort (mapcar #'column-index columns) #'<)))
    (or (gethash column-numbers cache)
        (let ((entropy 0.0d0)
              (nr-cells (apply #'* (mapcar #'nr-unique-values columns)))
              (ln-n (log (nr-rows computer))))
          (loop for i below nr-cells
               do (let* ((f (frequency-for-columns columns *data-source*))
                         (partial-entropy (* f (- (log f) ln-n)))
    (loop for i from 1 to nr-rows
         do (setf (aref partial-entropy i)
                  (* i (- (log i) ln-n))))
